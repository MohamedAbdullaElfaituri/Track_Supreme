...
// Kullanıcı kayıt işlemlerini yöneten ekran widget'ı
...
class _SignupScreenState extends State<SignupScreen> {
  ...
  // Email/şifre ile kayıt işlemini yöneten metod
  Future<void> _handleSignup() async {
    ...
    try {
      ...
      if (user != null) {
        _navigateToHome(user.uid); // Başarılı kayıtta ana sayfaya yönlendir
      }
    } on AuthServiceException catch (e) {
      _showErrorSnackbar(e.message); // Hata mesajını göster
    } catch (e) {
  ...
  // GitHub ile giriş/kayıt işlemi
  Future<void> _handleGitHubSignup() async {
    setState(() => _isLoading = true);
    try {
      await _authService.startGitHubSignIn();
    } catch (e) {
      _showErrorSnackbar('Failed to initiate GitHub signup');
    } finally {
      setState(() => _isLoading = false);
    }
  }
                ...
                child: Form(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 24),
                      _buildSignupButton(), // Kayıt butonu
                      const SizedBox(height: 24),
                      _buildSocialLoginButtons(), // Sosyal giriş butonları
                      const SizedBox(height: 16),
                      _buildLoginRedirect(), // Giriş sayfasına yönlendirme
                    ],
                  ),
                ),
              ),
  ...
  // Sosyal giriş butonları widget'ı
  ...
  // Giriş sayfasına yönlendirme widget'ı
